...
- **フェーズ C**: サービス管理カード
  - 録音 GUI の 1 画面ダッシュボード内に録音カード・サービス管理カード・パイプライン進捗を配置
...
- **設定 GUI 変更**: 「保存先変更」ボタンで `recordings_dir` をフォルダ選択ダイアログから変更可能（`update_settings_path` を `config_loader.py` に追加）
...
- **外部起動サービス停止**: サービス管理カードから外部起動の Ollama / Pipeline / Open WebUI も PID 検出できる場合は停止可能
- **Open WebUI Knowledge 同期**: `sync_webui.py` による `data/notes/*.md` の同期、GUI 起動中の notes 自動監視、Open WebUI 起動後の未同期回収を追加
- **本番セットアップ安定化**: `setup.ps1 prod` の PowerShell param 配置と UTF-8 BOM を修正し、prod は PyTorch cu128 / Open WebUI 0.9.5 固定でセットアップ
...
- 最新コミットは `0117825 本番セットアップ手順を安定化`。GitHub に push 済み
- worktree `.claude/worktrees/affectionate-meninsky-78b76a`、`festive-payne-6134b5`、`gracious-banach-010b7d`、`naughty-bardeen-8d9e65` が残存
...
- **未処理 WAV**: 現状 `data/input/` 直下に通常ファイルはなし
- **隔離ファイル**: `data/input/failed/` に `rec_2026-05-15_140351.wav` / `rec_2026-05-16_225939.wav` と各 `.meta.json` が残存。`failed_files.json` も 2 件
- **リトライ記録**: `retry_count.json` には古い残骸を含む 11 件が残存。ただし実ファイルが投入フォルダ直下にないため、現在のアクティブな自動リトライ対象は 0 件
...
- **NAS / SMB パスの決定と設定変更**
  - 手元 PC: `paths.recordings_dir` を NAS パスに（GUI の「保存先変更」で可）
...
- **隔離中 WAV のトリアージ**
  ...
  - 現在 `data/input/failed/` に残る実ファイルは `rec_2026-05-15_140351.wav` と `rec_2026-05-16_225939.wav`
  - 再試行する場合は GUI の「隔離ファイル」から投入フォルダへ戻す
...
- **VRAM 競合の手動回避ルール**: README と GUI は「Open WebUI の画面表示」ではなく「Open WebUI 経由のチャットで Ollama/Gemma が動くこと」が競合要因と明記。サービス管理カードから自動的にロックする仕組みはなし。警告ラベルのみ
